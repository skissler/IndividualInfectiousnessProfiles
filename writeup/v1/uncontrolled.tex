...
\begin{figure}[ht] 
\centering
\includegraphics[width=\textwidth]{figures/pdf/survival.pdf}
...
\subsection{Narrow individual infectiousness profiles yield greater uncertainty in epidemic growth rate and generation interval distribution estimates}
...
\begin{figure}[ht] 
\centering
\includegraphics[width=\textwidth]{figures/pdf/growthrate.pdf}
\caption{{\bf \boldmath Variation in estimated growth rate as a function of $\psi$.}  }
\label{fig:growthrate}
\end{figure}

A similar challenge arises when inferring the generation interval distribution, $g(\tau)$. Simulations show that, for a fixed number of index cases, the precision of estimtes for the distribution parameters $\alpha$ and $\beta$ widens as $\psi \rightarrow 0$. The reasoning for this is the share of the generation interval's variance that is attributed to within- \vs between-individual variation. In the smooth case, all the variation in $g(\tau)$ comes from within-individual variation in infection times --- each person's infectiousness profile is a copy of the population-level intrinsic generation interval distribution. Thus, each secondary infection is an independent sample from $g(\tau)$, providing maximal information about the shape of $g(\tau)$. At the opposite extreme, in the fully punctuated case, all the variation in $g(\tau)$ comes from between-individual variation. Secondary infection times from a single index case are perfectly correlated (identical in timing), and thus are completely redundant with one another. So, in the smooth case, the effective sample size obtained by recruiting $k$ index cases is $k R_0$; this decreases to simply $k$ in the fully punctuated case. Between $\psi = 0$ and $\psi = 1$, this scales as XXX. So, to achieve a similar precision in the estimates of the shape of $g(\tau)$, the number of index cases needed to recruit grows as XXX. 

Importantly, due to delays and uncertainty in infection ascertainment/timinng, it may not be clear whether the individual infectiousnes profile is punctuated or smooth: delays yield variation in the secondary infection timing, and when we see that variation, we won't immediately know if that's from variation in the timing of infections themselves or simply variation in when the infections were ascertaned. So, it is in general difficult to know how to power one's efforts to estimate the generation interval distribution shape beforehand. We turn to the question of estimating $\psi$ itself in Section XX. 
...
\end{equation}
which has mean $E[Z_i] = R_0$ and variance $R_0 (1 + R_0 / k)$. Thus, $k$ captures overdispersion, with $k \rightarrow \infty$ yielding Poisson-distributed $Z_i$ and $k \rightarrow 0$ yielding a higher likelihood of ``extreme'' secondary infection counts, \ie, superspreading. For convenience, we introduce the parameter $\text{OD} = 1/k$, such that large OD corresponds to high overdispersion. 
...
For the periodic contact model, we can derive a simple closed-form expression for $OD = 1/k$ in terms of the model parameters: 
%
\begin{equation}
...
\end{equation}
%
where $c_{amp}$ and $c_{per}$ are the amplitude and period of the contact process, respectively. This reveals a few key features about the relationship between $\psi$ and $k$. First, we note that $c_{amp} \rightarrow 0$ yields $\text{OD} \rightarrow 0$; \ie, in the degenerate case where contacts do not vary over time, we recover the standard Poisson-distributed secondary infection distribution with no overdispersion. Also, $\text{OD}$ is strictly decreasing in $\psi$, so more punctuated profiles ($\psi \rightarrow 0$) yield more overdispersion. The sensitivity of $\text{OD}$ to $\psi$, however, depends on the magnitude of $\eta = 2 \pi \bar{g} / (\alpha c_{per})$. When $\eta \gg 1$, which happens \eg when the mean generation time is much longer than the contact process' period ($\bar{g} \gg c_{per}$), $\text{OD}$ is small regardless of $\psi$. Thus, there is little overdispersion unless $\psi$ is very close to 0. When $\eta \ll 1$, which can happen when $\bar{g}$ is small relative to $c_{per}$, $\text{OD} \approx c_{amp}^2 / 2$ regardless of $\psi$. So, the impact of $\psi$ is greatest when $\eta \approx 1$, in which case $\psi \rightarrow 1$ causes the infectiousness profile to effectively average over the contact variation, while $\psi \rightarrow 0$ translates the contact variability into meaningful overdispersion. For our benchmark pathogens, $\bar{g}$ ranges from 3.2 days to 12.2 days. Thus, weekly variation in contacts ($c_{per} = 7$) causes $\text{OD}$ to vary meaningfully with $\psi$. Daily variation ($c_{per} = 1$) gets averaged out, yielding little overdispersion unless $\psi \approx 0$. Monthly variation ($c_{per} = 30$) leads to $\text{OD} \approx c_{amp}^2 / 2$ regardless of $\psi$. 

\begin{figure}[ht] 
\centering
\includegraphics[width=\textwidth]{figures/pdf/heatmaps.pdf}
