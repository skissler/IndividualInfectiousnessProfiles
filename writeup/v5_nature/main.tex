...
A simulation-based power analysis indicates that $\psi$ is identifiable from contact-tracing data under reasonably favorable conditions: if the distribution of uncertainty in infection times is known, 50 index cases sufficied for well-powered inference of $\psi$ when $R_0 \geq 6$ (\eg, SARS-CoV-2 or measles). Inference was more challenging for lower $R_0$, where fewer secondary cases per index case limit the amount of information available on the correlation of infection times within clusters. 
