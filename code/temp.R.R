...
# For psi=0.5: concentrate in [0.4, 0.6] (interval test)
psi_thresholds <- list(
	"0"   = list(type = "upper_tail", bound = 0.2),
	"0.5" = list(type = "interval",   lo = 0.4, hi = 0.6),
	...
	for (p in parslist) {
		...
		for (obs in obs_delay_scenarios) {
			...
			for (psi_true in psi_true_vals) {
				for (K in K_vals) {
					threshold <- psi_thresholds[as.character(psi_true)]
					...
					rep_results <- parallel::mclapply(seq_len(n_replicates), function(rep) {
						...
						tp <- compute_tail_prob(clusters, engines, psi_grid, threshold)
					...
					# Progress: show fraction that excluded
					if (psi_true < threshold) {
						frac_excluded <- mean(rep_df$tail_prob < tail_cutoff, na.rm = TRUE)
					} else {
						frac_excluded <- mean((1 - rep_df$tail_prob) < tail_cutoff, na.rm = TRUE)
					}
...
# tail_prob = P(psi > threshold | data), where threshold depends on psi_true
# psi_true = 0, threshold = 0.2: exclude psi > 0.2 => tail_prob < 0.05
# psi_true = 1, threshold = 0.8: exclude psi < 0.8 => (1 - tail_prob) < 0.05
results_df <- results_df %>%
	mutate(
		threshold = psi_thresholds[as.character(psi_true)],
		excluded = case_when(
			psi_true < threshold ~ tail_prob < tail_cutoff,
			psi_true > threshold ~ (1 - tail_prob) < tail_cutoff,
			TRUE ~ NA
		)
	)
...
obs_labels <- c(fast = "Fast detection (mean 2d)",
                moderate = "Moderate detection (mean 4d)",
                slow = "Slow detection (mean 8d)")
...
save_fig(fig1, "psi_identifiability_power", width = 14, height = 8)
